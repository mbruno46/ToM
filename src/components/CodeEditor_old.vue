...
var user_typing = {active: false, index: 0, pos: 0, timer: null, text: '', time: 250};
...
export default {
  ...
  setup() {
    ...
    //     else {
    //       history.checkpoint(caret.index, caret.pos, '\\n')
    //     }
    //   }
    // }
    ...
    function deleteTextAtCaret(len) {
      let caret = c.getCaret();
      c.setCaret(lines[caret.index], caret.pos, len);
      deleteSelectedText();
    }
    ...
    function History() {
      ...
      function newState(i, p, t, ot) {
        return {index: i, pos: p, text: t, oldtext: ot};
      }

      function setState(undo=true) {
        let s = stack[at];
        var nt;
        if (undo) {
          nt = s.oldtext.split(/\r?\n/);
          c.setCaret(lines[s.index], s.pos, s.text.length);
        } else {
          nt = s.text.split(/\r?\n/);
          c.setCaret(lines[s.index], s.pos, s.oldtext.length);
        }
        console.log(s, nt)
        deleteSelectedText();
        for (var i=0;i<nt.length;i++) {
          if (i>0) insertNewLine(false);
          insertTextAtCaret(nt[i]);
        }
      }
      ...
      return {
        ...
        checkpoint(index, pos, text, oldtext='') {
          at++;
          // let caret = c.getCaret();
          stack.splice(at, stack.length - at, 
            newState(index, pos, text, oldtext)
          );
          if (at==NMAX) {
            stack.splice(0,1);
            at--;
  ...
  methods: {
    ...
    handleInput: function(event) {
      ...
      console.log(event);
      // history
      if (!user_typing.active) {
        let caret = c.getCaret()
        user_typing.index = caret.index;
        user_typing.pos = caret.pos - 1;
        user_typing.active = true;
        user_typing.text = event.data;
      } else {
        user_typing.text += event.data;
      }
      clearTimeout(user_typing.timer);
      user_typing.timer = setTimeout(()=>{
        history.checkpoint(user_typing.index, user_typing.pos, user_typing.text);
        user_typing.active = false;
      }, user_typing.time);
    ...
    handleKeyDown: function(event) {
      ...
      if (event.key == "Tab") {
        prevent = true;
        if (event.shiftKey) this.removeTab();
        else this.insertTab();
      }
    ...
    refreshEditor(lines) {
      ...
      this.focus();
      history.checkpoint(this.getText());
    },
    getText() {
      var text = ''
      lines.forEach(line => {
